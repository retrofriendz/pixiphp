...
// From: PHP Cookbook (O'Reilly) 
if ( !function_exists('passwordcheck($user,$pass)') ) {
function passwordcheck($user,$pass) {
    ...
    if (strlen($pass) < 6) return 'The password is too short.';
    ...
    // two different kinds
    $max = $j - 2;
    if ($uc > $max) return "The password has too many upper case characters.";
    if ($lc > $max) return "The password has too many lower case characters.";
    if ($num > $max) return "The password has too many numeral characters.";
    if ($other > $max) return "The password has too many special characters.";
    return FALSE;
}
}
