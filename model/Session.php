<?php
 ...
 define('SESSION_TIMEOUT',"now +10 minutes");
 ...
 class Session extends Model {
  ...
  static public function ExpireExpired() {
   ...
   $m->Delete("expiresAt < ".strtotime('now'));
  }

  static public function CreateNew( $user ) {
   if ( intval($user["emailVerified"]) !== 1 ) return FALSE;
   ...
   $new_id=$m->Insert(array(
    'r_User'        => $user['ID'],
    'expiresAt'     => ($expiry=strtotime(SESSION_TIMEOUT))
   ));
   User::LastIP($user,$_SERVER['REMOTE_ADDR']);
  ...
  static public function Refresh() {
   ...
   $m->Update(array('expiresAt'=>strtotime(SESSION_TIMEOUT)),array('ID'=>$session_id));
   return TRUE;
  }
